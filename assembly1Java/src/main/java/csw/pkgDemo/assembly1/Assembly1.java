...
public class Assembly1 extends JAssemblyControllerWithLifecycleHandler {
  ...
  public Assembly1(AssemblyInfo info, ActorRef supervisor) {
    ...
    receive(defaultReceive().orElse(ReceiveBuilder.
      ...
      matchEquals(JSupervisor3.Running, x -> log.info("Received running")).
      matchEquals(JSupervisor3.RunningOffline, x -> log.info("Received running offline")).
      matchEquals(JSupervisor3.DoRestart, x -> log.info("Received dorestart")).
      matchEquals(JSupervisor3.DoShutdown, x -> {
        log.info("Received doshutdown");
        // Just say complete for now
        supervisor.tell(JSupervisor3.ShutdownComplete, self());
      }).
      match(Supervisor3.LifecycleFailureInfo.class, x -> log.info("Received failed state: " + x.state() + "for reason: " + x.reason())).
